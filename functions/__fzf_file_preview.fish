...
function __fzf_file_preview -d "Preview file in side panel"
    if test (count $argv) -eq 0
        echo "if test -d '{}'; then "(__fzf_dir_preview)" {}; elif file --mime {} | grep -iq binary; then if which hexyl 2>/dev/null; then hexyl -n 512 --color always {}; else xxd -l512 {}; fi; elif which bat 2>/dev/null; then bat --line-range :200 --paging never --color always {}; else head -n 30 {}; fi"
    else
