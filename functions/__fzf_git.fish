...
function __fzf_gf
  ...
  eval (__fzfcmd) -m --ansi --nth 2..,.. \
    --preview (string escape -n -- 'git diff --color=always -- {-1} | sed 1,4d; if file --mime {-1} | grep -iq directory; then "(__fzf_dir_preview)" {-1}; elif file --mime {-1} | grep -iq binary; then if which hexyl 2>/dev/null; then hexyl -n 512 --color always {-1}; else xxd -l512 {-1}; fi; elif which bat 2>/dev/null; then bat --line-range :200 --paging never --color always {-1}; else head -n 30 {-1}; fi') |
  cut -c4- | sed 's/.* -> //'
end
